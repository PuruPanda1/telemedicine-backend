...
public class AppointmentController {
    ...
    public ResponseEntity<List<AppointmentWithPatientDetailsDto>> getAllDoctorAppointments(@RequestParam Long doctorId) {
        List<Appointment> appointments = appointmentRepository.findAppointmentsByDoctorId(doctorId);

        List<AppointmentWithPatientDetailsDto> enrichedAppointments = appointments.stream()
                .map(appointment -> {
                    String patientName = patientClient.getPatientNameById(appointment.getPatientId()).getBody();
                    return new AppointmentWithPatientDetailsDto(appointment, patientName);
                })
                .toList();
